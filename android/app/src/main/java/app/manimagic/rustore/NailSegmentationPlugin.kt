...
class NailSegmentationPlugin : Plugin() {
    ...
    companion object {
        ...
        // ВАЖНО: это же число должно совпадать с входом .onnx, иначе модель
        // получит не тот тензор.
        //
        // Здесь 576, хотя веса обучены при 512: сеть свёрточная, вход другого
        // размера ей подать можно, и на реальных телефонных кадрах, где ноготь
        // мелкий, это помогает. Цена — макро: там ноготь и так крупный, вход
        // побольше раздувает его, и модель начинает мазать по коже.
        //
        // Замер 9 сентября, одни и те же веса:
        //   вход  контроль   подборка  макро     лишнее на макро
        //   512   115/133    12/32     58/59      21 331 px
        //   576   119/133    21/32     55/59      36 536 px
        //   640   124/133    21/32     53/59      73 157 px
        // Владелец увидел на телефоне ровно ту мазню, что стоит за 73 тысячами,
        // поэтому взят средний: почти весь выигрыш на обычных снимках и вдвое
        // меньше лишнего. Возврат к 512 — правка этой строки и контрольных
        // сумм в обоих рабочих процессах сборки.
        private const val INPUT_SIZE = 576
