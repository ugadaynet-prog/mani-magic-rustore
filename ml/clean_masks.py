...
def clean(rgb, mask):
    ...
    # Второй проход — по размеру. Замер 8 сентября по ручному эталону (39
    # кадров) показал, что он вреден: без него находится 209 ногтей из 225
    # против 204, а лишних пятен столько же. Правило писалось, когда модель
    # красила мох и жемчуг, и теперь бьёт по крупному ногтю большого пальца.
    # Оставлено выключенным, чтобы замер можно было повторить.
    if AREA_OUTLIER_ON and len(kept) >= AREA_OUTLIER_MIN_PARTS:
