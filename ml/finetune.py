...
BATCH = 4
DARK_P = 0.35
# Доля кадров, которым внутри маски рисуется узор вместо ровного цвета.
# Замер 8 сентября: при 0.35 форма улучшается (IoU 0.859 → 0.886), но полнота
# падает (91.4% → 86.4%), а жалоба владельца — именно про пропущенные ногти.
# Поэтому по умолчанию узора нет: ровно та сборка, что стоит в приложении.
# Проверять другую дозу — переменной окружения, а не правкой этой строки.
