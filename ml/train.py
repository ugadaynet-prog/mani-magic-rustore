...
# Сторона входа. Меняется переменной окружения SIZE — от неё зависят и
# обучение, и экспорт в ONNX, и они обязаны совпадать. Замер 6 сентября по
# ручному эталону: ноготь крупнее 400 пикселей на входе модель находит в 95%
# случаев, мельче 200 — в 67%, и именно мелкие составляют почти все потери.
# При 512 тот же ноготь получает в 1.78 раза больше пикселей.
# ВАЖНО: величина продублирована в Kotlin-плагине (INPUT_SIZE); если менять
# здесь, менять и там, иначе приложение подаст модели картинку не того размера.
